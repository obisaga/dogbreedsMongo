...
dogsRouter.post("/", async (req, res, next) => {
    try {
      const { breedName, dogImg, dogAbout, sportNeed, affectionateLevel } =
        req.body;
...
dogsRouter.get("/name/:breedName", async (req, res, next) => {
    try {
      ...
      if (!response) {
        res
          .status(404)
          .json({ message: `Dog with a name ${breedName} doesn't exist` }); //checks if the dog breed is the database
      } else {
        res.status(200).json(response);
      }
    } catch (err) {
...
dogsRouter.get("/id/:id", async (req, res, next) => {
    try {
      ...
      if (!id) {
        return res
          .status(404)
          .json({ message: `Dog with id ${id} doesn't exist` });
      }
    ...
    const response = await Dog.findOneAndUpdate(
      { breedName },
      {
        $set: {
          ...
          dogAbout: newAbout,
          sportNeed: newSport,
          affectionateLevel: newAffection,
        },
      },
      { new: true }
    );
